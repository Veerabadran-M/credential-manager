...
This is the only file that walks the plugins/ package. Everything else asks
this module for a backend by name (get_backend), a list of usable backends
(available_backends), or "the one to use if nothing more specific was
requested" (default_backend / resolve_backend).

Discovery is defensive on purpose: importing a plugin module must never be
allowed to crash the application, even if its dependency is broken or half
installed. A plugin that fails to import is simply skipped.
"""
