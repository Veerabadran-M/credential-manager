...
`credmgr init` fetches a much larger, real-world word list from a public
source and stores it at `<master_dir>/data/wordlist.txt` (see
datasources.py). WORD_LIST below is only the fallback used when that file
is missing or unreadable -- e.g. an offline first run -- so the tool never
becomes uninstallable without network access.
"""
