...
Checks: duplicate passwords (reused across accounts), weak passwords
(short/common/low diversity/patterned), reused passwords (matches an
account's own history), old passwords (older than
config.password_max_age_days), breached passwords (matched against a
local, offline SHA-1 hash database).

The datasets these checks use are fetched by `credmgr init` (see
datasources.py) into `<master_dir>/data/`; the constants below are only
the small offline fallback. Adding a check: write a `find_*` function
and wire it into run_audit() and AuditReport.
"""
