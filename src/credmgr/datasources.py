"""Fetches bundled security datasets -- wordlist, common passwords,
keyboard-sequence patterns, breached-password hashes -- from public
sources into `<master_dir>/data/`.
...
a download fails, so the tool stays usable offline; fetching just
upgrades those defaults to larger, real-world datasets.

Sources: wordlist.txt (Google's 10k English words, first20hours/
google-10000-english), common_passwords.txt (SecLists'
10k-most-common.txt), breached_hash.txt (SHA-1 hashes computed locally
from SecLists' Pwdb_top-100000.txt, a curated subset -- the full HIBP
corpus is gigabytes), sequences.txt (bundled directly; no canonical
public dataset for keyboard walks like "qwerty").
"""
